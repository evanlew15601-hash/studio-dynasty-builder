...
import { MediaItem, MediaCampaign, GameState } from '@/types/game';
...
interface MediaResponseDashboardProps {
  ...
  onReputationUpdate: (change: number) => void;
}
...
export const MediaResponseDashboard: React.FC<MediaResponseDashboardProps> = ({
  ...
  const playerRelevantMedia = recentMedia.filter(item =>
  ...
  const handleMediaResponse = async (action: any, mediaItem: MediaItem) => {
    try {
      ...
      if (reaction.cost) {
        onBudgetUpdate(gameState.studio.budget - reaction.cost);
      }
      
      if (reaction.impact.reputationChange) {
        onReputationUpdate(reaction.impact.reputationChange);
      }
  ...
  const launchPRCampaign = () => {
    ...
    if (campaignForm.targetType === 'studio') {
      ...
      targets.talent = [gameState.talent[0].id]; // For demo, use first talent
    } else if (campaignForm.targetType === 'project' && gameState.projects.length > 0) {
      targets.projects = [gameState.projects[0].id]; // For demo, use first project
    }
    ...
    onBudgetUpdate(gameState.studio.budget - cost);
      ...
      <Tabs defaultValue="responses" className="space-y-4">
        ...
        <TabsContent value="responses">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Media Stories Requiring Response */}
            <Card>
              <CardHeader>
