...
import { legacyWeeklyTickSystem } from '@/game/systems/legacyWeekly';
...
export const StudioMagnateGame: React.FC<StudioMagnateGameProps> = ({
  ...
  const registerSystems = useGameStore((s) => s.registerSystems);
  const advanceWeek = useGameStore((s) => s.advanceWeek);
  const advanceWeekAsync = useGameStore((s) => s.advanceWeekAsync);
  const tickId = useGameStore((s) => s.tickId);
  const lastTickReport = useGameStore((s) => s.lastTickReport);

  useEffect(() => {
    registerSystems([legacyWeeklyTickSystem]);
  }, [registerSystems]);
  ...
  // Post-tick persistence — run only when a week has actually advanced.
  useEffect(() => {
    if (!storeGameState) return;
    if (tickId <= 0) return;

    try {
      syncAndPersistIndustryDatabase('slot1', storeGameState);
    } catch (e) {
      console.warn('Failed to persist industry database', e);
    }
  }, [tickId, storeGameState]);

  // Auto-open Week Recap when the store publishes a new report.
  useEffect(() => {
    if (!lastTickReport) return;
  ...
  const handleAdvanceWeek = async (options?: { suppressToast?: boolean; suppressLoading?: boolean; suppressDiagnostics?: boolean; suppressRecap?: boolean }) => {
    ...
    try {
      await advanceWeekAsync({ suppressRecap: options?.suppressRecap });
    } finally {
      if (loadingEnabled) {
  ...
  const handleAdvanceWeeks = (weeks: number) => {
    ...
    const step = () => {
      ...
      advanceWeek({ suppressRecap: true });
      remaining -= 1;
