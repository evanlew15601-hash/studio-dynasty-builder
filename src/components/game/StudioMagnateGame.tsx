...
import type { StudioProfile } from '../../data/StudioGenerator';
...
export const StudioMagnateGame: React.FC<StudioMagnateGameProps> = ({
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const profileCache = new Map<string, StudioProfile | undefined>();
      const getProfile = (studioName: string) => {
        if (profileCache.has(studioName)) return profileCache.get(studioName);
        const profile = sg.getStudioProfile(studioName, mods);
        profileCache.set(studioName, profile);
        return profile;
      };
      ...
      for (const year of yearsToSeed) {
        for (let w = 1; w <= 52; w++) {
          ...
          for (const st of competitorStudios) {
            const profile = getProfile(st.name);
          ...
          if (releasesThisWeek === 0 && competitorStudios[0]) {
            const fallback = getProfile(competitorStudios[0].name);
            if (fallback) {
