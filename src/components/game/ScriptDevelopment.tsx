...
export const ScriptDevelopment: React.FC<ScriptDevelopmentProps> = ({
  ...
  // Auto-fill script based on selected franchise or public-domain IP
    ...
    if (selectedFranchise) {
      ...
      if (franchise) {
        return {
          ...
          budget: Math.max(5_000_000, franchise.culturalWeight * 500_000),
          ...
          characteristics: {
            ...
            visualStyle:
              franchise.genre.includes('fantasy') || franchise.genre.includes('sci-fi')
                ? 'stylized'
                : 'realistic',
            ...
            cgiIntensity:
              franchise.genre.includes('action') ||
              franchise.genre.includes('sci-fi') ||
              franchise.genre.includes('fantasy')
                ? 'heavy'
                : 'minimal',
          },
    ...
    if (selectedPublicDomain) {
      ...
      if (pd) {
        return {
          ...
          budget: Math.max(3_000_000, pd.reputationScore * 200_000),
          developmentStage: 'concept',
          ...
          characteristics: {
            ...
            visualStyle:
              pd.domainType === 'mythology' || pd.domainType === 'folklore'
                ? 'stylized'
                : 'realistic',
  ...
  useEffect(() => {
    ...
    const tempScript: Script = {
      ...
      genre: (next.genre as Genre) || 'drama',
      logline: next.logline || '',
      writer: next.writer || '',
      pages: next.pages || 120,
      quality: next.quality || 50,
      budget: next.budget || 5_000_000,
      developmentStage: (next.developmentStage as Script['developmentStage']) || 'concept',
      themes: next.themes || [],
      targetAudience: (next.targetAudience as Script['targetAudience']) || 'general',
      estimatedRuntime: next.estimatedRuntime || 120,
      characteristics:
        (next.characteristics as Script['characteristics']) || {
          tone: 'balanced',
          pacing: 'steady',
          dialogue: 'naturalistic',
          visualStyle: 'realistic',
      ...
      publicDomainId: next.publicDomainId,
      adaptationType: next.adaptationType,
      characters: [],
    };
  ...
  }, [selectedFranchise, selectedPublicDomain, editingScriptId, gameState]);
  ...
  useEffect(() => {
    ...
    setNewScript({
      ...
      adaptationType: existing.adaptationType,
    });
  ...
  const handleCreateScript = () => {
    if (!newScript.title || !newScript.logline || !newScript.genre) {
      toast({
        title: 'Incomplete Script',
        description: 'Please provide at least a title, genre, and logline.',
        variant: 'destructive',
      });
      return;
    }
  ...
  const availableScripts = gameState.scripts.filter(
    script => !gameState.projects.some(project => project.script.id === script.id),
  );
          ...
          <div className="text-sm text-blue-800">
            {selectedFranchise &&
              (() => {
                ...
                const projects = gameState.projects.filter(
                  p => p.script.franchiseId === franchise.id,
                );
                const filmCount = projects.filter(
                  p => p.type !== 'series' && p.type !== 'limited-series',
                ).length;
                const tvCount = projects.filter(
                  p => p.type === 'series' || p.type === 'limited-series',
                ).length;
                const totalBoxOffice = projects.reduce(
                  (sum, p) => sum + (p.metrics?.boxOfficeTotal || 0),
                  0,
                );
                const totalStreamingViews = projects.reduce(
                ...
                return (
                  <>
                    ...
                    <span className="ml-2 text-xs text-blue-900">
                      ...
                      {(totalBoxOffice / 1_000_000).toFixed(1)}M box office ·{' '}
                      {(totalStreamingViews / 1_000_000).toFixed(1)}M streams)
                    </span>
                  </>
                );
              })()}
      ...
      <div className="flex items-center justify-between animate-slide-up">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Create, refine, and greenlight your next productions
          </p>
        </div>
        <Button onClick={() => setIsCreating(true)} className="btn-studio animate-glow">
          <ScriptIcon className="mr-2" size={18} />
          New Script
        </Button>
      </div>

      {/* Script Creation / Editing Modal */}
      {isCreating && (
          ...
          <CardContent className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                {(newScript.sourceType === 'public-domain' || selectedPublicDomain) && (
                  <div>
                    <Label htmlFor="adaptation-style">Adaptation Style</Label>
                    <Select
                      value={newScript.adaptationType || 'faithful'}
                      onValueChange={value =>
                        setNewScript(prev => ({
                          ...prev,
                          adaptationType: value as Script['adaptationType'],
                        }))
                      }
                    >
                      <SelectTrigger className="mt-1">
                      ...
                      <SelectContent>
                        <SelectItem value="faithful">Faithful to Source</SelectItem>
                        <SelectItem value="modern">Modernized</SelectItem>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="mt-2">
                    <Slider
                      value={[newScript.budget || 5_000_000]}
                      onValueChange={([value]) =>
                        setNewScript(prev => ({
                          ...prev,
                          budget: value,
                    ...
                    <div className="flex justify-between text-xs text-muted-foreground mt-1">
                      ...
                      <span className="font-mono">
                        ${( (newScript.budget || 5_000_000) / 1_000_000 ).toFixed(0)}M
                      </span>
                      <span>$200M</span>
                    </div>
                  </div>
            ...
            <div className="flex justify-end space-x-3">
              ...
              <Button onClick={handleCreateScript} className="btn-studio animate-glow">
                ...
                >
                  <CardHeader className="pb-3">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      {script.sourceType === 'public-domain' && script.adaptationType && (
                        <Badge variant="outline" className="text-[10px] uppercase tracking-wide">
                          {script.adaptationType}
                        </Badge>
                      )}
                    </div>
                  </CardHeader>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-2 text-xs">
                        <div className="flex justify-between">
                          ...
                          <span className="font-mono">
                            ${(script.budget / 1_000_000).toFixed(1)}M
                          </span>
                        </div>
                        <div className="flex justify-between">
                ...
                {(newScript.sourceType === 'public-domain' || selectedPublicDomain) && (
                  <div>
                    ...
                    <Select
                      value={newScript.adaptationType || 'faithful'}
                      onValueChange={(value) =>
                        setNewScript(prev => ({ ...prev, adaptationType: value as any }))
                      }
                    >
                      <SelectTrigger className="mt-1">
                        <SelectValue />
