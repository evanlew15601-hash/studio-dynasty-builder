...
describe('media content templates modding', () => {
  ...
  it('replaces placeholder variants (case/spacing/underscores) in modded templates', () => {
    vi.spyOn(Math, 'random').mockReturnValue(0);
    ...
    const mods: ModBundle = {
      ...
      patches: [
        {
          id: 'p-headlines-variants',
          modId: 'my-mod',
          entityType: 'mediaHeadlineTemplates',
          op: 'update',
          target: 'casting_announcement',
          payload: ["MODDED: {actor name} cast in {studio_name}'s {film_title}"],
        },
        {
          id: 'p-content-variants',
          modId: 'my-mod',
          entityType: 'mediaContentTemplates',
          op: 'update',
          target: 'casting_announcement',
          payload: ['CONTENT: {ACTOR_NAME} joins {Film Title} ({genre})'],
        },
      ],
    };
    ...
    expect(item.headline).toContain('MODDED:');
    expect(item.headline).toContain('Jamie Star');
    expect(item.headline).toContain('Player Studio');
    expect(item.headline).toContain('Player Premiere');

    expect(item.content).toContain('CONTENT:');
    expect(item.content).toContain('Jamie Star');
    expect(item.content).toContain('Player Premiere');
    expect(item.content).toContain('action');
